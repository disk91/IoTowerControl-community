...
```json
{
  ...
  "profile": {
        ...
        "timezone": "string",         // user timezone ex: CEST
    ...
    "billingProfile": {
        ...
        "countryCode": "string",      // user country code (extension for more precision) [Base64(encrypted)]
    ...
    "customFields": [{                  // user custom fields
        "name": "string",               // custom field key [clear], the one starting with `basic_` or `cbasic_` or `chide_` are returned in the basic API
